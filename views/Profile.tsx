  ...
  return (
    <div className="view-transition max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-12 flex flex-col lg:flex-row gap-6 sm:gap-8 min-h-[85vh] bg-black relative overflow-x-hidden">
      ...
      <aside className="w-full lg:w-[280px] shrink-0 relative z-10">
        <div className="bg-gradient-to-br from-[#0a0a0a] to-[#050505] border border-white/5 rounded-3xl flex flex-col shadow-xl lg:sticky lg:top-24 overflow-hidden">
          {/* User Profile Summary */}
          <div className="p-5 pb-6 border-b border-white/10 space-y-4 hidden lg:block">
          ...
          <div className="flex lg:flex-col overflow-x-auto lg:overflow-visible p-2 lg:p-5 gap-2 lg:gap-1 no-scrollbar">
            {menuItems.map((item) => (
              <button
                key={item.id}
                onClick={() => setActiveTab(item.id as any)}
                className={`shrink-0 flex items-center justify-between px-4 lg:px-5 py-3 lg:py-3 rounded-xl transition-all group ${activeTab === item.id
              ...
              >
                ...
                {item.badge && (
                  <span className={`px-2 py-0.5 rounded-full text-[8px] font-black ml-2 ${activeTab === item.id
          ...
          <div className="hidden lg:block h-px bg-white/10 my-4 mx-5"></div>

          <div className="hidden lg:flex flex-col gap-1 px-5 pb-5">
            ...
            <button
              ...
              className="flex items-center justify-between px-5 py-3 text-white/50 hover:text-white hover:bg-white/5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all"
            >
